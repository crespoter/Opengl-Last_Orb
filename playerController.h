...
enum positionStats {MOVE_LEFT,MOVE_RIGHT,STOP};
const float minPos = -0.65;
const float maxPos = 0.65;
...
public:
	...
	glm::mat4 Update(float deltaTime)
	{
		...
		case MOVE_LEFT:
			...
			model = glm::translate(model, glm::vec3(moveLeft(minPos, deltaTime), 0.0, 0.0));
			model = glm::rotate(model, glm::radians(startGanking(ROTATING_LEFT, deltaTime, minRotation, maxRotation)), glm::vec3(0.0f, 1.0f, 0.0f));
			break;
		case MOVE_RIGHT:
			rotationAngle = rotationAngle < 0 ? 0 : rotationAngle;
			model = glm::translate(model, glm::vec3(moveRight(maxPos, deltaTime) , 0.0, 0.0));
			model = glm::rotate(model, glm::radians(startGanking(ROTATING_RIGHT, deltaTime, minRotation, maxRotation)), glm::vec3(0.0f, 1.0f, 0.0f));
			break;
		default:
